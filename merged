...
void receivedCallback( uint32_t from, String &msg ) {
 ...
 if(msg == "online"){
  ...
  if (mcp == 1 && mfd == 1 )
  {
    ...
    msgMfd = readMfd();
    mesh.sendSingle(root, msgMfd );                                        // Send msg to single node. To broadcast msg (mesh.sendBroadcast(msg)) 
...
String readMfd(){
 ...
 int i=100,j=0;
  while(i<160)
  {
    ...
    msgMfd = id   ;     
    msgMfd +=  "," + String(readWattage());              //KW
    msgMfd += "," + String(readWattageY());                //wy
    msgMfd += "," + String(readWattageR());//WR
    msgMfd += "," + String(readWattageB());//WB
    msgMfd +=  "," + String(readCurrent());//Iavg
    msgMfd += ","  + String(readLineVoltage()) ; //Vll
    msgMfd += ","  + String(readWattHour()) ;//pf
    msgMfd += ","  + String(readPf()) ;//pf
    msgMfd += "," + String(readVa());//KVA
    msgMfd += "," + String(readVah());//VAH
